...
import com.github.gianlucafattarsi.liberapi.application.exception.type.LibraryPathNotConfiguredException;
...
@ControllerAdvice
public class LibraryPathNotConfiguredExceptionHandler {

    private final LocaleUtilsMessage localeUtilsMessage;

    @ExceptionHandler(LibraryPathNotConfiguredException.class)
    public ResponseEntity<?> handleExpiredTokenException(LibraryPathNotConfiguredException ex,
        ...
        final ErrorData errorData = ErrorData.builder()
                                             ...
                                             .userMessage(localeUtilsMessage.getMessage(
                                                     "library.not.configured.detail",
                                                     null,
                                                     request))
                                             .userTitle(localeUtilsMessage.getMessage(
                                                     "library.not.configured.title",
                                                     null,
                                                     request))
                                             .build();
